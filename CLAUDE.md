...
Command-ness is line shape, never membership of the advertised list (measured
provider interception behaviour: `provider-system` skill). Per-site rationale —
the escape hatch that lets the kind be re-derived from the text, the
once-per-submit state that moved to the first prose block, `async` local
