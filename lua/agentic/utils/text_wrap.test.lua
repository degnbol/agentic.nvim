...
describe("agentic.utils.TextWrap", function()
    ...
    describe("truncate_to_width", function()
        it("leaves a string that already fits", function()
            assert.equal("short", TextWrap.truncate_to_width("short", 10))
            assert.equal("exact", TextWrap.truncate_to_width("exact", 5))
        end)

        it("cuts to the budget with the ellipsis in the last cell", function()
            local out = TextWrap.truncate_to_width("abcdefghij", 6)
            assert.equal("abcde…", out)
            assert.equal(6, vim.fn.strdisplaywidth(out))
        end)

        it("keeps double-width characters inside the budget", function()
            local out = TextWrap.truncate_to_width(string.rep("世", 10), 7)
            assert.equal(7, vim.fn.strdisplaywidth(out))
        end)

        it("gives up under four columns rather than say nothing", function()
            assert.equal("abcdef", TextWrap.truncate_to_width("abcdef", 3))
        end)
    end)
    ...
    describe("abbreviate_count", function()
        it("spells out a count below a thousand", function()
            ...
            assert.equal("420", TextWrap.abbreviate_count(420))
            assert.equal("999", TextWrap.abbreviate_count(999))
        end)

        it("suffixes a thousand and above", function()
            assert.equal("1.0k", TextWrap.abbreviate_count(1000))
            assert.equal("1.2k", TextWrap.abbreviate_count(1234))
            assert.equal("12.3k", TextWrap.abbreviate_count(12345))
        end)
    end)

    describe("paragraph_break", function()
        it("prepends a blank line after an open line", function()
            assert.equal("\n\nfoo", TextWrap.paragraph_break("foo", 0))
        end)

        it("completes a blank line after one newline", function()
            assert.equal("\nfoo", TextWrap.paragraph_break("foo", 1))
        end)

        it("adds nothing after a blank line", function()
            assert.equal("foo", TextWrap.paragraph_break("foo", 2))
            assert.equal("foo", TextWrap.paragraph_break("foo", 3))
        end)
    end)
end)
