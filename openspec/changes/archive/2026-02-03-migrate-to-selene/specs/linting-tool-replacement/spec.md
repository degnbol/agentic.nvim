# Spec: Linting Tool Replacement

## Capability

`linting-tool-replacement`

## Overview

Replace luacheck with selene as the Lua linting tool across all development
environments (local, CI, Docker) to provide faster, more accurate, and better
maintained linting capabilities.
...
None - this change adds a new tool, it doesn't modify existing requirements.

---

## REMOVED Requirements

### Requirement: Luacheck must be the primary Lua linting tool (REMOVED)

This requirement is completely replaced by the "Selene must be the primary Lua
linting tool" requirement above.

**Rationale:** Luacheck is unmaintained (last update 2018) and lacks modern
features like type checking and rich error reporting.

---

### Requirement: Makefile must provide luacheck targets (REMOVED)

Replaced by "Makefile must provide selene targets" requirement.
...
**Rationale:** Targets are renamed to match new tool, functionality preserved.

---

### Requirement: Inline lint directives must use luacheck syntax (REMOVED)

Replaced by "Inline lint directives must use selene syntax" requirement.

**Old syntax:** `-- luacheck: ignore 542`

**New syntax:** `-- selene: allow(empty_if)`

**Rationale:** Selene uses descriptive names instead of numeric codes for
...
None - this is a standalone capability with no dependencies on other OpenSpec
changes.

## Notes

- Selene 0.30.0 was released on 2026-01-22
- Selene is written in Rust with multithreading support
- Luacheck last updated October 2018 (8+ years unmaintained)
- Migration should be straightforward due to minimal configuration needs
- Performance improvement expected due to Rust implementation
